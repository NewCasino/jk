                            ...
                            if($this->session->userdata('usertype') != 'employer'){
                                if($this->session->userdata('usertype') == 'member'){ ?>
                                        <?php if($jobApplyExists){ ?>
                                            <a href="#" class="btn btn-xs btn-danger btn-block" disabled>APPLICATION EXISTS</a>    
                                        <?php }else{ ?>
                                            <a href="<?= BASEURL . 'members/applyJob/'.$jobInfo['jobDetails']['jobId'].'/'.$jobInfo['jobDetails']['empId'] ?>" class="btn btn-xs btn-primary btn-block">APPLY</a>
                                        <?php } ?>
                                <?php }else{ ?>
                                    <a href="<?= BASEURL . 'ajkk/applyNow/'.$jobInfo['jobDetails']['jobId'] ?>" class="btn btn-xs btn-primary">APPLY</a>
                                    <a href="<?= BASEURL . 'auth/register/member' ?>" class="btn btn-xs btn-success">BOOKMARK</a>
                        <?php }
                        } ?>
                    </div>
                </div>
                ...
                <div class="panel-body">
                    <div class="media">
                        <div class="media-body">
                            <h4 class="media-heading page-header text-uppercase">Company Name:</h4> <?= strtoupper($jobInfo['jobDetails']['comp_name']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Nature of Business:</h4> <?= ucwords($jobInfo['jobDetails']['comp_nature']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Business Reg #:</h4> <?= ucwords($jobInfo['jobDetails']['business_regno']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Company Type:</h4> <?= ucwords($jobInfo['jobDetails']['comp_type']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Company Size:</h4> <?= ucwords($jobInfo['jobDetails']['comp_size']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Dress Code:</h4> <?= ucwords($jobInfo['jobDetails']['dress_code']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Working Hrs:</h4> <?= ucwords($jobInfo['jobDetails']['working_hrs']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">Spoken Language:</h4> <?= ucwords($jobInfo['jobDetails']['spoken_lang']) ?><br/><br/>
                            <?php if($jobInfo['jobDetails']['organization'] != ''){ ?><h4 class="media-heading page-header text-uppercase">Organization:</h4> <?= ucwords($jobInfo['jobDetails']['organization']) ?><br/><br/><?php } ?>
                            <?php if($jobInfo['jobDetails']['website'] != ''){ ?><h4 class="media-heading page-header">Website:</h4> <?= $jobInfo['jobDetails']['website'] ?><br/><br/><?php } ?>
                            <h4 class="media-heading page-header text-uppercase">Address:</h4> <?= ucwords($jobInfo['jobDetails']['address'].','.$jobInfo['jobDetails']['city'].', '.$jobInfo['jobDetails']['country']) ?><br/><br/>
                            <h4 class="media-heading page-header text-uppercase">About Us:</h4>
                            <strong><?= $jobInfo['jobDetails']['comp_desc'] ?></strong></div>
